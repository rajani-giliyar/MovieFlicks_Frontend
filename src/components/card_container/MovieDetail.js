...
const MovieDetails = () => {
    ...
    const fetchMovieDetails = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/movies/getMovieDetailsById/${id}`);
    ...
    // If movie details are not loaded yet, display a loading message
    if (!movieDetails) {
    ...
    const handleGoBack = () => {
        navigate("/home"); // Go back to the previous page (Home page)
    };
        ...
        <div style={styles.container}>
            ...
            <div style={styles.detailsContainer}>
                <div style={styles.imageContainer}>
                    <img src={movieDetails.image} alt={movieDetails.movieName} style={styles.image} />
                </div>
